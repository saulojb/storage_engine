 ...
 */
static IndexScanDesc
engine_index_beginscan(Relation heapRelation,
						 ...
						 Bitmapset *attr_needed)
{
	...
	scan->xs_heapfetch = engine_index_fetch_begin_extended(heapRelation, attr_needed);
...
IndexNext(IndexScanState *node)
{
	...
	if (scandesc == NULL)
	{
		...
		scandesc = 
			engine_index_beginscan(node->ss.ss_currentRelation,
...
.CustomName = "ColcompressIndexScan",
...
ExplainPropertyText("ColcompressIndexScan using ", indexname, es);
...
CustomScan *
engine_create_indexscan_node(void)
{
...
void
engine_register_indexscan_node(void)
{
	/* Guard against double-registration with Citus/TimescaleDB columnar */
	if (GetCustomScanMethods(ColumnarIndexCustomScanMethods.CustomName, true) == NULL)
