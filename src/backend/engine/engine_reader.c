				  ...
				  ParallelColumnarScan parallelColumnarScan)
{
	...
	if (!randomAccess)
	{
		 ...
		 * (i.e.: SeqScan and Engine CustomScan).
		 *
...
ColumnarEndRead(ColumnarReadState *readState)
{
	...
	{
		/*
		 * init_engine_read_state created a new snapshot and registered it,
		 * so now forget it.
		 */
...
AdvanceStripeRead(ColumnarReadState *readState)
{
	...
	else
	{
		...
		/*
		 * Atomically claim the next stripe index from the pre-fetched list.
		 * No spinlock needed -- pg_atomic_fetch_add_u64 is already atomic,
		 * and each worker claims a unique index so there is no duplication.
		 */
		uint64 myIndex = pg_atomic_fetch_add_u64(
			&readState->parallelColumnarScan->nextStripeIndex, 1);

		if (myIndex >= readState->parallelColumnarScan->stripeCount)
		{
			/* All stripes have been claimed; this worker has no more work. */
			readState->currentStripeMetadata = NULL;
		}
		else
		{
			uint64 myStripeId = readState->parallelColumnarScan->stripeIds[myIndex];

			/*
			 * Fetch full stripe metadata from the catalog.  This is done
			 * outside any spinlock -- catalog access (table_open, index scan
			 * etc.) while holding a spinlock causes a "stuck spinlock" PANIC
			 * because spinlocks must never block on I/O or catalog pins.
			 */
			uint64 foundStripeId = myStripeId;
			readState->currentStripeMetadata = FindNextStripeForParallelWorker(
				readState->relation, readState->snapshot,
				myStripeId, &foundStripeId);

			/*
			 * Safety check: if the returned stripe has a different ID, our
			 * pre-fetched stripe was removed by a concurrent VACUUM.  Skip
			 * it; another worker holds the actual next stripe in its slot.
			 */
			if (readState->currentStripeMetadata != NULL &&
				readState->currentStripeMetadata->id != myStripeId)
			{
				readState->currentStripeMetadata = NULL;
			}
		}
