...
typedef struct ColumnarScanDescData
{
	 ...
	 */
#if PG_VERSION_NUM >= PG_VERSION_18
	/* PG18+: single callback, iterator lives in cs_base.st.rs_tbmiterator */
	bool          cs_bm_have_page;      /* true while we have an active TBM block */
	TBMIterateResult cs_tbmres;         /* current TBM block result */
	OffsetNumber *cs_bm_offsets;        /* extracted exact offsets (allocated lazily) */
	int           cs_bm_noffsets;       /* # of exact offsets (-1 = lossy) */
	int           cs_bm_offidx;         /* index into cs_bm_offsets[] */
	uint64        cs_bm_rownum;         /* next row# for lossy scan */
	uint64        cs_bm_rownum_end;     /* exclusive end row# for lossy scan */
#else
	/* PG17: two callbacks; tbmres is passed by caller */
	TBMIterateResult *cs_tbmres;        /* current TBM block (caller-owned) */
	int           cs_bm_offidx;         /* index into tbmres->offsets[] */
	uint64        cs_bm_rownum;         /* next row# for lossy scan */
	uint64        cs_bm_rownum_end;     /* exclusive end row# for lossy scan */
#endif
...
typedef struct IndexFetchColumnarData
{
	...
	/*
	 * We initialize cs_readState lazily in the first engine_index_fetch_tuple
	 * call. However, we want to do memory allocations in a sub MemoryContext of
	 * engine_index_fetch_begin. For this reason, we store scanContext in
	 * engine_index_fetch_begin.
	 */
	MemoryContext scanContext;
...
#if PG_VERSION_NUM < PG_VERSION_18
static bool engine_scan_bitmap_next_block(TableScanDesc sscan,
											struct TBMIterateResult *tbmres);
static bool engine_scan_bitmap_next_tuple(TableScanDesc sscan,
											struct TBMIterateResult *tbmres,
											TupleTableSlot *slot);
#else
static bool engine_scan_bitmap_next_tuple(TableScanDesc sscan,
...
engine_beginscan(Relation relation, Snapshot snapshot,
				   ...
				   uint32 flags)
{
	...
	/* For TableAM parallel scan, cast the generic pscan descriptor to our type */
	ParallelColumnarScan parallelColumnarScan =
		(parallel_scan != NULL) ? (ParallelColumnarScan) parallel_scan : NULL;

	TableScanDesc scandesc = engine_beginscan_extended(relation, snapshot, nkeys, key,
														 parallel_scan,
...
TableScanDesc
engine_beginscan_extended(Relation relation, Snapshot snapshot,
							int nkeys, ScanKey key,
							ParallelTableScanDesc parallel_scan,
...
{
	return AllocSetContextCreate(CurrentMemoryContext, "Engine Scan Context",
								 ALLOCSET_DEFAULT_SIZES);
}


/*
 * init_engine_read_state initializes a column store table read and returns the
 * state.
 */
...
static void
engine_endscan(TableScanDesc sscan)
{
...
static void
engine_rescan(TableScanDesc sscan, ScanKey key, bool set_params,
...
static bool
engine_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	ColumnarScanDesc scan = (ColumnarScanDesc) sscan;
...
{
	/*
	 * Pre-count the stripes so we can allocate exactly the right amount of
	...
	if (stripeCount < 1)
		stripeCount = 1; /* always allocate at least one slot */

	return MAXALIGN(offsetof(ParallelColumnarScanData, stripeIds) +
...
{
	ParallelColumnarScan pcscan = (ParallelColumnarScan) pscan;

	/*
	 * Pre-fetch all stripe IDs for this relation into the shared DSM so that
	 * workers can claim them atomically without any catalog access in the
	...
	foreach(lc, stripeList)
	{
		...
		pcscan->stripeIds[i++] = stripe->id;
	}
	pcscan->stripeCount = stripeCount;
	pg_atomic_init_u64(&pcscan->nextStripeIndex, 0);

	/*
	 * Initialise the standard PG18 parallel scan header.
	...
	pcscan->base.phs_snapshot_off = MAXALIGN(
		offsetof(ParallelColumnarScanData, stripeIds) + stripeCount * sizeof(uint64));
...
static void
engine_parallelscan_reinitialize(Relation rel, ParallelTableScanDesc pscan)
{
	ParallelColumnarScan pcscan = (ParallelColumnarScan) pscan;
	pg_atomic_write_u64(&pcscan->nextStripeIndex, 0);
}
...
IndexFetchTableData *
engine_index_fetch_begin_extended(Relation rel, Bitmapset *attr_needed)
{
#if PG_VERSION_NUM >= PG_VERSION_16
...
static void
engine_index_fetch_end(IndexFetchTableData *sscan)
{
	engine_index_fetch_reset(sscan);
...
static bool
engine_index_fetch_tuple(struct IndexFetchTableData *sscan,
						   ItemPointer tid,
						   Snapshot snapshot,
						   TupleTableSlot *slot,
						   bool *call_again, bool *all_dead)
{
	...
	*call_again = false;
	 ...
	 * require implementing engine_index_delete_tuples for simple deletion
	...
	if (scan->cs_readState == NULL)
	{
		...
		scan->cs_readState = init_engine_read_state(columnarRelation,
...
static bool
engine_fetch_row_version(Relation relation,
						   ItemPointer tid,
						   Snapshot snapshot,
						   TupleTableSlot *slot)
{
	...
	if (readState == NULL)
	{
		...
		*readState = init_engine_read_state(relation,
											  slot->tts_tupleDescriptor,
...
static void
engine_get_latest_tid(TableScanDesc sscan,
						ItemPointer tid)
{
	elog(ERROR, "engine_get_latest_tid not implemented");
}


static bool
engine_tuple_tid_valid(TableScanDesc scan, ItemPointer tid)
{
	elog(ERROR, "engine_tuple_tid_valid not implemented");
}


static bool
engine_tuple_satisfies_snapshot(Relation rel, TupleTableSlot *slot,
								  Snapshot snapshot)
{
...
engine_index_delete_tuples(Relation rel,
							 TM_IndexDeleteOp *delstate)
{
	...
	else
	{
		 ...
		 * columnar tables (bottomup = false), engine_index_fetch_tuple
		 ...
		 * the safe side.
		 */
		elog(ERROR, "engine_index_delete_tuples not implemented for simple deletion");
	}
}
...
#else
static TransactionId
engine_compute_xid_horizon_for_tuples(Relation rel,
										ItemPointerData *tids,
										int nitems)
{
	elog(ERROR, "engine_compute_xid_horizon_for_tuples not implemented");
}
					  ...
					  int options, BulkInsertState bistate)
{
	...
	/*
	 * engine_init_write_state allocates the write state in a longer
...
static void
engine_tuple_complete_speculative(Relation relation, TupleTableSlot *slot,
...
#endif
{
	...
	engine_tuple_insert(relation, slot, cid, 0, NULL);
...
static TM_Result
engine_tuple_lock(Relation relation, ItemPointer tid, Snapshot snapshot,
...
static void
engine_relation_set_new_filenode(Relation rel,
...
static void
engine_relation_nontransactional_truncate(Relation rel)
{
#if PG_VERSION_NUM >= PG_VERSION_16
...
static void
engine_relation_copy_data(Relation rel, const RelFileLocator *newrnode)
{
	elog(ERROR, "engine_relation_copy_data not implemented");
}


/*
 * engine_relation_copy_for_cluster is called on VACUUM FULL, at which
					...
					BufferAccessStrategy bstrategy)
{
	...
	/*
	 * Engine storage doesn't hold any transaction IDs, so we can always
...
#if PG_VERSION_NUM >= PG_VERSION_18
	/* all frozen pages are always 0 for columnar */
	...
	engine_enable_page_cache = old_cache_mode;
}
...
LogRelationStats(Relation rel, int elevel)
{
	...
	Datum storageId = DirectFunctionCall1(se_engine_relation_storageid,
...
static double
engine_index_build_range_scan(Relation columnarRelation,
								Relation indexRelation,
								IndexInfo *indexInfo,
								bool allow_sync,
								bool anyvisible,
								...
								TableScanDesc scan)
{
	...
	{
		/*
		 * Engine utility hook already errors out for BRIN indexes on columnar
	...
	if (scan != NULL)
	{
		 ...
		 * progress reporting.
		 */
		snapshot = ((ColumnarScanDesc) scan)->cs_base.rs_snapshot;
	}
	else
	{
		/*
		 * Serial index build: set up a snapshot and begin our own full-table
		 * scan. We use SnapshotAny when we have a valid OldestXmin (normal
		 * VACUUM context), or a registered MVCC snapshot for concurrent
		 * builds and bootstrap.
		 */
						  ...
						  ExprState *predicate)
{
	...
	while (engine_getnextslot(scan, ForwardScanDirection, slot))
	{
		CHECK_FOR_INTERRUPTS();
		...
		{
			/*
			 * engine_getnextslot guarantees that returned tuple will
...
static void
engine_index_validate_scan(Relation columnarRelation,
							 Relation indexRelation,
							 IndexInfo *indexInfo,
							 Snapshot snapshot,
							 ValidateIndexState *
...
static uint64
engine_relation_size(Relation rel, ForkNumber forkNumber)
{
	uint64 nblocks = 0;
...
static bool
engine_relation_needs_toast_table(Relation rel)
{
	return false;
}
...
static void
engine_estimate_rel_size(Relation rel, int32 *attr_widths,
...
/*
 * engine_scan_bitmap_next_block (PG17 only)
 *
 * Called once per bitmap page/block before scan_bitmap_next_tuple is called
 * for each tuple in that block.  We only save the TBMIterateResult pointer
 * and reset intra-block state; actual row fetching is deferred to the tuple
 * callback so that we use a single code path on all PG versions.
 */
#if PG_VERSION_NUM < PG_VERSION_18
static bool
engine_scan_bitmap_next_block(TableScanDesc sscan,
								struct TBMIterateResult *tbmres)
{
	ColumnarScanDesc scan = (ColumnarScanDesc) sscan;

	scan->cs_tbmres    = tbmres;
	scan->cs_bm_offidx = 0;

	if (tbmres->ntuples < 0)
	{
		/* Lossy block: we must visit every possible row in this block */
		scan->cs_bm_rownum     = (uint64) tbmres->blockno * VALID_ITEMPOINTER_OFFSETS;
		scan->cs_bm_rownum_end = scan->cs_bm_rownum + VALID_ITEMPOINTER_OFFSETS;
	}

	return true;  /* block may contain visible tuples */
}


/*
 * engine_scan_bitmap_next_tuple (PG17)
 *
 * Fetch the next visible tuple from the current bitmap block.
 * Uses ColumnarReadRowByRowNumber for random-access row lookup.
 */
static bool
engine_scan_bitmap_next_tuple(TableScanDesc sscan,
								struct TBMIterateResult *tbmres,
								TupleTableSlot *slot)
{
	ColumnarScanDesc scan = (ColumnarScanDesc) sscan;

	/* Initialise the columnar read state on the first call */
	if (scan->cs_readState == NULL)
	{
	...
	while (true)
	{
		...
		if (tbmres->ntuples < 0)
		{
			/* Lossy block: iterate through all row-numbers in the block */
			if (scan->cs_bm_rownum >= scan->cs_bm_rownum_end)
				return false;
			rowNumber = scan->cs_bm_rownum++;
		}
		else
		{
			/* Exact block: only the listed offsets */
			if (scan->cs_bm_offidx >= tbmres->ntuples)
				return false;
			OffsetNumber off = tbmres->offsets[scan->cs_bm_offidx++];
		...
		if (!ColumnarReadRowByRowNumber(scan->cs_readState, rowNumber,
									   slot->tts_values, slot->tts_isnull))
			continue;  /* gap in row numbers — not an actual tuple */

		slot->tts_tableOid = RelationGetRelid(scan->cs_base.rs_rd);
		slot->tts_tid      = row_number_to_tid(rowNumber);
		ExecStoreVirtualTuple(slot);
		return true;
	}
}
#else  /* PG_VERSION_NUM >= PG_VERSION_18 */

/*
 * engine_scan_bitmap_next_tuple (PG18+)
 *
 * In PG18 the two-callback split was merged into a single callback that owns
 * the full block-iteration loop.  The TBMIterator is embedded in the base
 * scan descriptor (scan->cs_base.st.rs_tbmiterator) and was initialised by
 * the executor before this callback was first invoked.
 */
static bool
engine_scan_bitmap_next_tuple(TableScanDesc sscan,
								TupleTableSlot *slot,
								...
								uint64 *exact_pages)
{
	...
	if (scan->cs_bm_offsets == NULL)
	{
		MemoryContext old = MemoryContextSwitchTo(scan->scanContext);
		scan->cs_bm_offsets = (OffsetNumber *)
	...
	while (true)
	{
		/* ---- Advance to the next TBM block when the current one is done ---- */
		if (!scan->cs_bm_have_page)
		{
			if (!tbm_iterate(&sscan->st.rs_tbmiterator, &scan->cs_tbmres))
				return false;  /* bitmap exhausted */

			scan->cs_bm_have_page = true;
			*recheck = scan->cs_tbmres.recheck;

			if (scan->cs_tbmres.lossy)
			{
				/* Lossy block: must visit every possible row */
				scan->cs_bm_rownum     = (uint64) scan->cs_tbmres.blockno
										  * VALID_ITEMPOINTER_OFFSETS;
				scan->cs_bm_rownum_end = scan->cs_bm_rownum + VALID_ITEMPOINTER_OFFSETS;
				scan->cs_bm_noffsets   = -1;  /* sentinel for lossy */
				(*lossy_pages)++;
			}
			else
			{
		...
		/* ---- Fetch the next row from the current block ---- */
		uint64 rowNumber;

		if (scan->cs_bm_noffsets == -1)  /* lossy */
		{
			if (scan->cs_bm_rownum >= scan->cs_bm_rownum_end)
			{
				scan->cs_bm_have_page = false;
				continue;
			}
			rowNumber = scan->cs_bm_rownum++;
		}
		...
		{
			if (scan->cs_bm_offidx >= scan->cs_bm_noffsets)
			{
				scan->cs_bm_have_page = false;
				continue;
			}
			OffsetNumber off = scan->cs_bm_offsets[scan->cs_bm_offidx++];
			rowNumber = (uint64) scan->cs_tbmres.blockno * VALID_ITEMPOINTER_OFFSETS
						+ (uint64) (off - FirstOffsetNumber);
		}
...
#endif  /* PG_VERSION_NUM >= PG_VERSION_18 */


static bool
engine_scan_sample_next_block(TableScanDesc scan, SampleScanState *scanstate)
{
	elog(ERROR, "engine_scan_sample_next_block not implemented");
}


static bool
engine_scan_sample_next_tuple(TableScanDesc scan, SampleScanState *scanstate,
								TupleTableSlot *slot)
{
	elog(ERROR, "engine_scan_sample_next_tuple not implemented");
}
...
ColumnarSlotCopyHeapTuple(TupleTableSlot *slot)
{
	...
	/* slot->tts_tid is filled in engine_getnextslot */
...
/*
 * IsColumnarTableAmTable returns true if relation has engine_tableam
...
static const TableAmRoutine engine_am_methods = {
	.type = T_TableAmRoutine,

	.slot_callbacks = engine_slot_callbacks,

	.scan_begin = engine_beginscan,
	.scan_end = engine_endscan,
	.scan_rescan = engine_rescan,
	.scan_getnextslot = engine_getnextslot,

	.parallelscan_estimate = engine_parallelscan_estimate,
	.parallelscan_initialize = engine_parallelscan_initialize,
	.parallelscan_reinitialize = engine_parallelscan_reinitialize,

	.index_fetch_begin = engine_index_fetch_begin,
	.index_fetch_reset = engine_index_fetch_reset,
	.index_fetch_end = engine_index_fetch_end,
	.index_fetch_tuple = engine_index_fetch_tuple,

	.tuple_fetch_row_version = engine_fetch_row_version,
	.tuple_get_latest_tid = engine_get_latest_tid,
	.tuple_tid_valid = engine_tuple_tid_valid,
	.tuple_satisfies_snapshot = engine_tuple_satisfies_snapshot,
#if PG_VERSION_NUM >= PG_VERSION_14
	.index_delete_tuples = engine_index_delete_tuples,
#else
	.compute_xid_horizon_for_tuples = engine_compute_xid_horizon_for_tuples,
#endif

	.tuple_insert = engine_tuple_insert,
	.tuple_insert_speculative = engine_tuple_insert_speculative,
	.tuple_complete_speculative = engine_tuple_complete_speculative,
	.multi_insert = engine_multi_insert,
	.tuple_delete = engine_tuple_delete,
	.tuple_update = engine_tuple_update,
	.tuple_lock = engine_tuple_lock,
	.finish_bulk_insert = engine_finish_bulk_insert,

#if PG_VERSION_NUM >= PG_VERSION_16
	.relation_set_new_filelocator = engine_relation_set_new_filenode,
#else
	.relation_set_new_filenode = engine_relation_set_new_filenode,
#endif
	.relation_nontransactional_truncate = engine_relation_nontransactional_truncate,
	.relation_copy_data = engine_relation_copy_data,
	.relation_copy_for_cluster = engine_relation_copy_for_cluster,
	.relation_vacuum = engine_vacuum_rel,
	.scan_analyze_next_block = engine_scan_analyze_next_block,
	.scan_analyze_next_tuple = engine_scan_analyze_next_tuple,
	.index_build_range_scan = engine_index_build_range_scan,
	.index_validate_scan = engine_index_validate_scan,

	.relation_size = engine_relation_size,
	.relation_needs_toast_table = engine_relation_needs_toast_table,

	.relation_estimate_size = engine_estimate_rel_size,

#if PG_VERSION_NUM < PG_VERSION_18
	.scan_bitmap_next_block = engine_scan_bitmap_next_block,
#endif
	.scan_bitmap_next_tuple = engine_scan_bitmap_next_tuple,
	.scan_sample_next_block = engine_scan_sample_next_block,
	.scan_sample_next_tuple = engine_scan_sample_next_tuple
};
...
{
	return &engine_am_methods;
}


PG_FUNCTION_INFO_V1(colcompress_handler);
Datum
colcompress_handler(PG_FUNCTION_ARGS)
{
	PG_RETURN_POINTER(&engine_am_methods);
}

/* Legacy alias so DROP EXTENSION columnar can still resolve this symbol */
PG_FUNCTION_INFO_V1(se_engine_handler);
Datum
se_engine_handler(PG_FUNCTION_ARGS)
{
	PG_RETURN_POINTER(&engine_am_methods);
}
...
/*
 * alter_engine_table_set is a UDF exposed in postgres to change settings on a columnar
 * table. Calling this function on a non-columnar table gives an error.
 *
 * sql syntax:
 *   pg_catalog.alter_engine_table_set(
 *        table_name regclass,
 ...
 * like:
 *   SELECT alter_engine_table_set('table', compression => 'pglz');
 *
 ...
 */
PG_FUNCTION_INFO_V1(se_alter_engine_table_set);
Datum
se_alter_engine_table_set(PG_FUNCTION_ARGS)
{
	...
	if (!PG_ARGISNULL(5))
	{
		text *orderbyText = PG_GETARG_TEXT_PP(5);
	...
	if (!PG_ARGISNULL(6))
	{
		options.indexScan = PG_GETARG_BOOL(6);
		ereport(DEBUG1, (errmsg("updating index_scan to %s",
										options.indexScan ? "on" : "off")));
	}
...
/*
 * alter_engine_table_reset is a UDF exposed in postgres to reset the settings on a
 * columnar table. Calling this function on a non-columnar table gives an error.
 *
 * sql syntax:
 *   pg_catalog.alter_engine_table_re
 *   teset(
 *        table_name regclass,
 ...
 * All arguments except the table name are optional. The UDF is supposed to be called
 * like:
 *   SELECT alter_engine_table_set('table', compression => true);
 *
 * All options set to true will be reset to the default system value.
 */
PG_FUNCTION_INFO_V1(se_alter_engine_table_reset);
Datum
se_alter_engine_table_reset(PG_FUNCTION_ARGS)
{
	Oid relationId = PG_GETARG_OID(0);
	...
	{
		options.chunkRowCount = engine_chunk_group_row_limit;
		ereport(DEBUG1,
	...
	{
		options.stripeRowCount = engine_stripe_row_limit;
		ereport(DEBUG1,
	...
	{
		options.compressionType = engine_compression;
	...
	{
		options.compressionLevel = engine_compression_level;
	...
	/* index_scan => true: reset to false (disabled) */
	if (!PG_ARGISNULL(6) && PG_GETARG_BOOL(6))
	{
		options.indexScan = false;
		ereport(DEBUG1, (errmsg("resetting index_scan to off")));
	}
...
/*
 * upgrade_engine_storage - upgrade columnar storage to the current
 * version.
 *
 * DDL:
 *   CREATE OR REPLACE FUNCTION upgrade_engine_storage(rel regclass)
 *     RETURNS VOID
 *     STRICT
 *     LANGUAGE c AS 'MODULE_PATHNAME', 'upgrade_engine_storage';
 */
PG_FUNCTION_INFO_V1(se_upgrade_engine_storage);
Datum
se_upgrade_engine_storage(PG_FUNCTION_ARGS)
{
...
/*
 * downgrade_engine_storage - downgrade columnar storage to the
 * current version.
 *
 * DDL:
 *   CREATE OR REPLACE FUNCTION downgrade_engine_storage(rel regclass)
 *     RETURNS VOID
 *     STRICT
 *     LANGUAGE c AS 'MODULE_PATHNAME', 'downgrade_engine_storage';
 */
PG_FUNCTION_INFO_V1(se_downgrade_engine_storage);
Datum
se_downgrade_engine_storage(PG_FUNCTION_ARGS)
{
	Oid relid = PG_GETARG_OID(0);
...
PG_FUNCTION_INFO_V1(se_vacuum_engine_table);
Datum
se_vacuum_engine_table(PG_FUNCTION_ARGS)
{
	Oid relid = PG_GETARG_OID(0);
...
PG_FUNCTION_INFO_V1(se_engine_stats);
Datum
se_engine_stats(PG_FUNCTION_ARGS)
{
...
/*
 * colcompress_merge re-writes all data of a colcompress table into a new set
 * of stripes, applying the sort order defined by the table's `orderby` option.
 *
 * If the table has orderby configured, the INSERT uses ORDER BY so that rows
 * arrive at the writer already globally sorted.  Each stripe then receives a
 * contiguous, non-overlapping range of key values, giving true global ordering
 * across all stripes.
 *
 * Usage:
 *   SELECT engine.colcompress_merge('schema.table'::regclass);
 */
PG_FUNCTION_INFO_V1(colcompress_merge);
Datum
colcompress_merge(PG_FUNCTION_ARGS)
{
Oid relationId = PG_GETARG_OID(0);

/* Open with AccessExclusiveLock – blocks concurrent reads/writes */
Relation rel = table_open(relationId, AccessExclusiveLock);

if (!IsColumnarTableAmTable(relationId))
{
table_close(rel, AccessExclusiveLock);
ereport(ERROR, (errmsg("table %s is not a colcompress table",
   quote_identifier(RelationGetRelationName(rel)))));
}
...
/* Read current options to get orderby string */
ColumnarOptions options = { 0 };
if (!ReadColumnarOptions(relationId, &options))
{
table_close(rel, AccessExclusiveLock);
ereport(ERROR, (errmsg("colcompress_merge: unable to read options for table")));
}

/* Build SQL-safe qualified relation name */
...
/* Unique temp table name based on OID (no conflicts across sessions) */
char tmpname[64];
snprintf(tmpname, sizeof(tmpname), "_ccmerge_%u", relationId);

/* Release relation object (keep AccessExclusiveLock via NoLock) */
table_close(rel, NoLock);
...
/* Step 1 – copy all rows to temp heap table */
...
ereport(ERROR, (errmsg("colcompress_merge: CREATE TEMP TABLE failed (code %d)", ret)));

/* Step 2 – truncate the target (resets stripes; options including orderby are preserved) */
resetStringInfo(&buf);
appendStringInfo(&buf, "TRUNCATE %s", qualname);

ret = SPI_execute(buf.data, false, 0);
if (ret != SPI_OK_UTILITY)
ereport(ERROR, (errmsg("colcompress_merge: TRUNCATE failed (code %d)", ret)));

/* Step 3 – insert back, with ORDER BY when orderby is configured.
 * Rows arrive at ColumnarWriteRow already globally sorted, so each stripe
 * receives a contiguous key range → true global ordering across stripes. */
resetStringInfo(&buf);
...
ereport(ERROR, (errmsg("colcompress_merge: INSERT failed (code %d)", ret)));

uint64 rowsMerged = SPI_processed;
...
ret = SPI_execute(buf.data, false, 0);
if (ret != SPI_OK_UTILITY)
ereport(ERROR, (errmsg("colcompress_merge: DROP TABLE failed (code %d)", ret)));

SPI_finish();

ereport(NOTICE,
			(errmsg("colcompress_merge: " UINT64_FORMAT " rows merged into %s%s",
					rowsMerged, qualname,
