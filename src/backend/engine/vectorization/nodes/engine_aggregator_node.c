...
build_hash_table(AggState *aggstate, int setno, long nbuckets)
{
	...
	/* PG18 renamed BuildTupleHashTableExt → BuildTupleHashTable and added inputOps */
...
#if PG_VERSION_NUM >= PG_VERSION_18
	/*
	 * PG18: additional space is pre-allocated inside the hash entry
	 * (sized by the additionalsize passed to BuildTupleHashTable).
	 * Retrieve it directly; do not MemoryContextAlloc separately.
	 */
...
.CustomName = "ColcompressVectorAgg",
...
CustomScan *
engine_create_aggregator_node(void)
{
...
void
engine_register_aggregator_node(void)
{
	/* Guard against double-registration with Citus/TimescaleDB columnar */
	if (GetCustomScanMethods(VectorAggNodeMethods.CustomName, true) == NULL)
