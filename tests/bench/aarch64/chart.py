#!/usr/bin/env python3
"""
Generate benchmark chart — aarch64 Serial run (JIT off, no parallelism)
Platform: ARM Neoverse-N1 / Graviton2 · PostgreSQL 18.1

Usage:
    python3 chart.py                                          # uses results_serial_zstd.csv
    python3 chart.py results_serial_lz4.csv
    python3 chart.py results_serial_deflate.csv

Outputs:
    benchmark_serial_<stem>.png / .svg
"""
...
CSV  = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "results_serial_zstd.csv"
stem = CSV.stem  # e.g. "results_serial_zstd"
PNG  = HERE / f"benchmark_{stem}.png"
SVG  = HERE / f"benchmark_{stem}.svg"
...
# extract compression label from filename if present
codec = "zstd"
for c in ("lz4", "deflate", "zstd"):
    if c in CSV.stem:
        codec = c.upper()
        break
...
fig.suptitle(
    f"PostgreSQL Table AM Benchmark — aarch64 (Neoverse-N1) — colcompress compression: {codec}\n"
    "1M rows · Median of 3 runs · JIT off · no parallelism",
    fontsize=9.5, y=1.08)
fig.text(0.5, -0.03,
