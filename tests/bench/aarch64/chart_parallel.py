#!/usr/bin/env python3
"""
Generate benchmark chart — aarch64 Parallel run (JIT on, 4 workers)
Platform: ARM Neoverse-N1 / Graviton2 · PostgreSQL 18.1

Usage:
    python3 chart_parallel.py                                     # uses results_parallel_zstd.csv
    python3 chart_parallel.py results_parallel_lz4.csv
    python3 chart_parallel.py results_parallel_deflate.csv

Outputs:
    benchmark_<stem>.png / .svg
"""
...
CSV  = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "results_parallel_zstd.csv"
stem = CSV.stem
...
fig.suptitle(
    ...
    "1M rows · Median of 3 runs · JIT on · 4 parallel workers",
    fontsize=9.5, y=1.08)
fig.text(0.5, -0.03,
