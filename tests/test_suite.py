...
class TestRunner:
    ...
    def test_extension_lifecycle(self) -> None:
        ...
        self.check("extension version = 2.3.0", ver == "2.3.0", f"got {ver!r}")
    ...
    def test_upgrade_path(self) -> None:
        ...
        self.check("latest available version = 2.3.0", ver == "2.3.0", f"got {ver!r}")

        # Complete upgrade path from 1.0 to 2.2.0 exists
        path = self.q1(
            "SELECT path FROM pg_extension_update_paths('storage_engine') "
            "WHERE source = '1.0' AND target = '2.3.0'"
        )
        self.check("upgrade path 1.0 → 2.3.0 exists", path != "", f"path={path!r}")
